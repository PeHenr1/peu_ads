...
public final class DeclaracaoCompleta extends Declaracao {
    ...
    public double getDespesaDedutivel(){
        var gastoEducacao = gastos.stream().filter(g -> g instanceof GastoEducacao ).mapToDouble(Gasto::getValor).sum();
        var gastoSaude = gastos.stream().filter(g -> g instanceof GastoSaude ).mapToDouble(Gasto::getValor).sum();

        return Math.min(gastoEducacao,GastoEducacao.MAX_GASTO_EDUCACAO) + Math.min(gastoSaude,GastoSaude.MAX_GASTO_SAUDE);
    }
    ...
    public String toString() {
        StringBuilder sb = new StringBuilder("Declaracao Completa");
